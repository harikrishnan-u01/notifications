            ...
            <div className="notificationfilter-container">
              <span>
                ...
                <select
                  ...
                  id="titles"
                  onChange={(selected) =>
                    this.getSelectedNotification(selected)
                  }
                >
              ...
              <span className="notification-add">
                <Button
                  variant="contained"
                  color="primary"
                  onClick={() => this.showAddNotification()}
                >
                  Add Notifications
                </Button>
              </span>
            </div>
            {this.isNotificationsExists() ? (
              this.state.filteredNotifications.map((item) => {
                return (
                  <Card
                      ...
                      className={classNames(
                        "notification-card-content-container",
                        {
                          ...
                          "notification-card-content-warning":
                            item.severity === "2",
                          "notification-card-content-info":
                            item.severity === "1",
                        }
                      )}
                    >
                      ...
                      <CardActions className="notification-card-delete">
                        <Button
                          size="small"
                          onClick={() => this.showEditNotification(item)}
                        >
                          <EditIcon />
                        </Button>
                      </CardActions>
